...
class SnappyRecyclerView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) :
    RecyclerView(context, attrs, defStyle), Snapper,
    MSnapper {
     ...
     */
    fun enableSnapListener(behavior: amarenkov.androidx.snappyrecyclerview.SnappyRecyclerView.Behavior) {
        ...
        when (behavior) {
            NOTIFY_ON_SCROLL -> addOnScrollListener(object : OnScrollListener() {
                ...
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                    if (dx == 0 && dy == 0) return
                    if (hasBeenDragged && currentPosition != NO_POSITION) {
                        hasBeenDragged = false
                        onNoItemCentered()
                    } else if (isVertical) {
    ...
    private fun onItemCentered() {
        ...
        if (currentPosition != position) {
            currentPosition = position
            mAdapter!!.onNewItemCentered(currentPosition)
        }
    }
    ...
    override fun getSnappedPosition(): Int {
        if (mIsScrollEnabled) return currentPosition
        if (layoutManager == null) return NO_POSITION
        val target = mSnapHelper.findSnapView(layoutManager) ?: return NO_POSITION
    ...
    private fun setCenteringPadding() {
        ...
        if (!isPaddingApplied && mIsPaddingSet) {
            ...
            var snapTo = 0
            if (currentPosition != NO_POSITION) snapTo =
                currentPosition
            snapTo(snapTo)
        }
    }
